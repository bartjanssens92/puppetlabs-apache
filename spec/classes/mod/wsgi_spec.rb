...
describe 'apache::mod::wsgi', :type => :class do
  ...
  context "on a Archlinux osfamily" do
    ...
    describe "with custom mod_path not containing /" do
      ...
      it { is_expected.to contain_apache__mod('wsgi').with({
          'package'  => 'mod_wsgi_package',
          'path'     => '/etc/httpd/modules/wsgi_mod_name.so',
        })
      }
      it { is_expected.to contain_file('wsgi.load').with_content(%r"LoadModule wsgi_module /etc/httpd/modules/wsgi_mod_name.so") }
